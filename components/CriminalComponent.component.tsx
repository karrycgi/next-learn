import { RedNoticeArrestWarrent, RedNoticeDetails, SexId } from "next-learn-red-notice-api";
...
const CriminalComponent = (criminal: RedNoticeDetails): JSX.Element => {
    return <div className={styles.wrapper}>
        <div className={styles.one}>
            ...
            {criminal.nationalities && <div><p>Staatsbürgerschaften</p> <ul>{criminal.nationalities.map((nat:string, index:number) => <li key={index}>{nat}</li>)}</ul> </div> }

            {criminal.height && <p>Größe: {criminal.height}</p> }
            {criminal.weight &&  <p>Gewicht: {criminal.weight} kg</p> }
            {criminal.hairs_id &&  <div><p>Haarfarben</p> <ul>{criminal.hairs_id.map((hair:string, index:number) => <li key={index}>{hair}</li>)}</ul> </div> }
            {criminal.eyes_colors_id &&  <div><p>Augenfarben</p> <ul>{criminal.eyes_colors_id.map((eye:string, index:number) => <li key={index}>{eye}</li>)}</ul> </div> }
            {criminal.languages_spoken_ids &&  <div><p>Sprachkenntnisse</p> <ul>{criminal.languages_spoken_ids.map((ls:string, index:number) => <li key={index}>{ls}</li>)}</ul> </div> }
        </div>
        ...
        <div className={styles.three}>
            ...
            </tr></thead> <tbody>{criminal.arrest_warrants.map((warrant:RedNoticeArrestWarrent, index:number) =><tr key={index}> <td>{warrant.issuing_country_id}</td> <td>{warrant.charge}</td></tr>)}</tbody> </table> }
        </div>
    </div>
}
